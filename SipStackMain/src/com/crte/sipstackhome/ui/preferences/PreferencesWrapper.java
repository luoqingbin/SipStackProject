...
public class PreferencesWrapper {
    ...
    //Internal use
    public static final String HAS_BEEN_QUIT = "has_been_quit"; // 退出状态
    ...
    public final static HashMap<String, String> STRING_PREFS = new HashMap<String, String>() {
        ...
        {
            // 更改用户的用户代理
            put(SipConfigManager.USER_AGENT, "CSipSimple"); /* 警告 */
...
//	}
	...
	private static File getStorageFolder(Context ctxt, boolean preferCache) {
	    ...
	    if (root.canWrite()){
			File dir = new File(root.getAbsolutePath() + File.separator + /*CustomDistribution.getSDCardFolder()*/ "SipStackHome");
			if(!dir.exists()) {
				dir.mkdirs();
				Log.d(THIS_FILE, "创建目录： " + dir.getAbsolutePath());
			}
			return dir;
	    }
	    return null;
