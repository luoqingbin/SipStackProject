...
import com.crte.sipstackhome.utils.Compatibility;
...
public class NativeLibManager {
	...
	public static File getLibFileFromPackage(ApplicationInfo appInfo, String libName, boolean allowFallback) {
		LogUtils.v(THIS_FILE, "Dir "+appInfo.dataDir);
		if(Compatibility.isCompatible(9)) {
			try {
				Field f = ApplicationInfo.class.getField("nativeLibraryDir");
				File nativeFile = new File((String) f.get(appInfo), libName);
				if(nativeFile.exists()) {
					LogUtils.v(THIS_FILE, "Found native lib using clean way");
					return nativeFile;
				}
			} catch (Exception e) {
				LogUtils.e(THIS_FILE, "Cant get field for native lib dir: " + e);
			}
		}
		if(allowFallback) {
	...
	public static boolean isDebuggableApp(Context ctx) {
		try {
			...
			LogUtils.e(THIS_FILE, "Not possible to find self name: " + e);
		}
		return false;
	}
